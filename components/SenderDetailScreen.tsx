...
const SenderDetailScreen: React.FC<SenderDetailScreenProps> = ({
  ...
  return (
    <div className="h-[calc(100vh-100px)] flex flex-col relative">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-slate-900">{senderGroup.senderName}</h1>
            <p className="text-slate-500">{senderGroup.senderEmail}</p>
          ...
          <button
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium flex items-center gap-2 transition-colors"
          >
            {isAnalyzingAll ? (
              <>
      ...
      <div className={`fixed bottom-6 left-1/2 transform -translate-x-1/2 w-full max-w-xl px-4 z-20 transition-opacity ${
        isTrashingSelected ? 'opacity-50 pointer-events-none' : ''
      }`}>
        <div className={`text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between transition-colors ${
